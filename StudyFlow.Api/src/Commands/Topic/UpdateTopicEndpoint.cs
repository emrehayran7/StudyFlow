...
namespace StudyFlow.Api.src.Commands.Topic
{
    public static class UpdateTopicEndpoint
    {
        ...
        {
            app.MapPut("/api/topics/{topicId:int}", async (int topicId, UpdateTopicDto dto, ClaimsPrincipal user, IMediator mediator) =>
            {
                int userId = user.GetUserId();
                UpdateTopicCommand command = new UpdateTopicCommand(topicId, dto, userId);
